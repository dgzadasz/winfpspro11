...
import { ArrowRight, Check, Download, ExternalLink, LockKeyhole, LogOut, ShieldCheck, Sparkles, WandSparkles } from "lucide-react";
...
type Order = { id: string; plan: string; planName: string; amountCents: number; status: "pending" | "approved"; createdAt: string; notifiedAt: string | null };
...
export default function Home() {
  ...
  useEffect(() => { refresh().catch(() => setMessage("Não foi possível carregar a sessão Discord.")).finally(() => setLoading(false)); }, []);
  ...
  return <div className="store-shell">
    <header className="topbar"><a className="brand" href="#top"><span className="brand-mark">SK$</span><span>STORE</span></a><nav><a href="#plans">Planos</a><a href="#packs">Packs</a><a href="#how">Como funciona</a><a href="#account">Minha conta</a></nav><div className="account-area">{loading ? <span className="muted">Carregando...</span> : user ? <><span className="user-name">{user.displayName}</span><Button variant="ghost" size="sm" onClick={logout}><LogOut size={16} /> Sair</Button></> : <Button onClick={login}>Entrar com Discord <ArrowRight size={16} /></Button>}</div></header>
    <main id="top">
      <section className="hero-section"><div className="hero-copy"><div className="eyebrow"><Sparkles size={14} /> ANDROID · ACESSO PREMIUM</div><h1>Seu próximo acesso.<br /><em>Começa aqui.</em></h1><p>Entre com Discord, escolha seu plano ou pack e pague com Pix. Cada pedido é registrado no canal de compras para conferência manual.</p><div className="hero-actions"><a className="primary-link" href="#plans">Ver planos <ArrowRight size={17} /></a><a className="secondary-link" href="#packs">Conhecer packs</a></div><div className="trust-row"><span><ShieldCheck size={16} /> Cliente verificado no Discord</span><span><LockKeyhole size={16} /> Aprovação manual</span></div></div><div className="hero-card"><div className="orb orb-a" /><div className="orb orb-b" /><div className="hero-card-content"><span className="card-kicker">SK$ / ACCESS</span><strong>ACESSO<br /><span>SEM LIMITES</span></strong><div className="card-footer"><span>ANDROID</span><span>2026</span></div></div></div></section>
      <section id="plans" className="content-section"><div className="section-heading"><div><span className="section-index">01 / LOJA</span><h2>Escolha seu plano</h2></div><p>Acesso por período com registro no Discord e confirmação manual.</p></div><div className="plans-grid">{plans.map((plan, index) => <article className={`plan-card ${plan.featured ? "featured" : ""}`} key={plan.key}>{plan.featured && <div className="popular">MAIS POPULAR</div>}<div className="plan-icon">0{index + 1}</div><h3>{plan.name}</h3><p>{plan.duration}</p><div className="price">{plan.price}</div><Link href={`/checkout/${plan.key}`} className="plan-button">Ir para compra <ArrowRight size={16} /></Link><ul><li><Check size={15} /> Pagamento via Pix</li><li><Check size={15} /> Registro no canal de compras</li></ul></article>)}</div></section>
      <section id="packs" className="content-section packs-section"><div className="section-heading"><div><span className="section-index">02 / PACKS</span><h2>Sensibilidade para seu setup.</h2></div><p>Compra única. Baixe o material aprovado e aprenda a ajustar com segurança.</p></div><div className="plans-grid packs-grid">{packs.map(pack => <article className={`plan-card pack-card ${pack.featured ? "featured" : ""}`} key={pack.key}>{pack.featured && <div className="popular">COM IA PREMIUM</div>}<div className="plan-icon"><WandSparkles size={16} /></div><h3>{pack.name}</h3><p>{pack.description}</p><div className="price">{pack.price}</div><Link href={`/checkout/${pack.key}`} className="plan-button">Ver checkout <ArrowRight size={16} /></Link><span className="pack-note">Download protegido após aprovação</span></article>)}</div></section>
