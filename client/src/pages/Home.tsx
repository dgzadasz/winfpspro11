...
export default function Home() {
  ...
  const [user, setUser] = useState<DiscordUser | null>(null); const [orders, setOrders] = useState<Order[]>([]); const [loading, setLoading] = useState(true); const [message, setMessage] = useState(""); const [items, setItems] = useState(cartCount());
  ...
  useEffect(() => { refresh().catch(() => setMessage("Não foi possível carregar a sessão Discord.")).finally(() => setLoading(false)); const onCart = () => setItems(cartCount()); window.addEventListener("sk-cart-updated", onCart); window.addEventListener("storage", onCart); return () => { window.removeEventListener("sk-cart-updated", onCart); window.removeEventListener("storage", onCart); }; }, []);
