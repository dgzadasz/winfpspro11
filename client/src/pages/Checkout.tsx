...
export default function Checkout() {
  ...
  const copy = async () => { if (!order) return; try { await navigator.clipboard.writeText(order.pix); setMessage("Pix copia e cola copiado."); } catch { setMessage("Não foi possível copiar automaticamente. Selecione o código Pix no campo acima e copie manualmente."); } };
  if (!selected) return <div className="checkout-page"><h1>Plano não encontrado</h1><Link href="/">Voltar para a loja</Link></div>;
  return <div className="checkout-page"><header className="checkout-header"><Link href="/" className="brand"><span className="brand-mark">SK$</span><span>STORE</span></Link><Link href="/" className="back-link"><ArrowLeft size={16} /> Voltar para a loja</Link></header><main className="checkout-main"><span className="section-index">CHECKOUT / PIX</span><h1>Finalize seu acesso.</h1><p className="checkout-intro">Seu pedido será registrado no Discord para conferência manual. Você pode reabrir o pagamento ou cancelar enquanto estiver pendente.</p><div className="checkout-layout"><section className="checkout-summary"><span className="plan-number">PLANO SELECIONADO</span><h2>{selected.name}</h2><p>{selected.duration}</p><strong>{selected.price}</strong><ul><li><Check size={16} /> Identidade verificada pelo Discord</li><li><Check size={16} /> Conferência do pedido pelo Discord</li><li><Check size={16} /> Confirmação manual, sem aprovação automática</li></ul></section><section className="checkout-payment">{mobilePack && !order && <DevicePicker value={profile} onChange={setProfile}/>} {loading ? <p>Verificando sua conta...</p> : !user ? <><h2>Entre para continuar</h2><p>Você precisa entrar com Discord para vincular a compra à sua conta.</p><Button onClick={login}>Entrar com Discord</Button></> : !order ? <><h2>Gerar pagamento</h2><p>Cliente: <strong>{user.displayName}</strong></p><Button onClick={generate} disabled={working || (mobilePack && !validDeviceProfile(profile))}>{working ? "Gerando QR Code..." : "Gerar QR Code Pix"}</Button></> : order.status === "approved" ? <><h2>Pagamento confirmado</h2><p>Seu acesso está disponível em Minha conta.</p><Link href="/dashboard" className="plan-button">Ver minha compra</Link></> : order.status === "cancelled" ? <><h2>Compra cancelada</h2><p>Este pedido foi encerrado. Cancelar o pedido não estorna um Pix já enviado; nesse caso, fale com o suporte.</p><Link href="/cart" className="plan-button">Voltar ao carrinho</Link></> : <><div className="payment-success"><Check size={18} /> Pedido pendente</div>{order.notified === false && <p role="alert">Seu pedido foi salvo, mas o aviso ao Discord falhou. Antes de pagar, <Link href="/support">fale com o suporte</Link> e informe o número abaixo.</p>}<h2>Escaneie para pagar</h2><img className="pix-qr" src={order.qr} alt="QR Code Pix" /><strong className="payment-value">{(order.amountCents / 100).toFixed(2).replace(".", ",")} BRL</strong><p className="order-reference">Pedido <b>{order.id}</b></p><textarea readOnly value={order.pix} aria-label="Pix copia e cola" /><div className="order-meta"><Button onClick={copy}><Clipboard size={16} /> Copiar Pix copia e cola</Button><Button variant="outline" onClick={cancel} disabled={working}><XCircle size={16} /> Cancelar compra</Button></div><p className="privacy-note"><LockKeyhole size={14} /> O código Pix não contém seu nome nem seu ID Discord.</p></>}</section></div>{message && <div className="checkout-feedback" role="status" aria-live="polite">{message}</div>}</main><footer><span>Pix: DIEGO · SAO PAULO</span><a href="https://discord.gg/xJY2PZ6Zx" target="_blank" rel="noreferrer">Suporte pelo Discord ↗</a></footer></div>;
}
