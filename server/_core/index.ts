...
async function startServer() {
  ...
  app.get("/api/store/licenses", async (req, res) => {
    const user = getDiscordUser(req);
    if (!user) return res.status(401).json({ error: "discord_login_required" });
    try { const db = await getDb(); if (!db) return res.status(503).json({ error: "licenses_unavailable" }); const admin = Boolean(process.env.DISCORD_ADMIN_ID && user.id === process.env.DISCORD_ADMIN_ID); const columns = sql`id, order_id AS "orderId", ${admin ? sql`discord_id AS "discordId",` : sql``} plan, CASE WHEN status = 'ACTIVE' AND expires_at IS NOT NULL AND expires_at <= now() THEN 'EXPIRED' ELSE status END AS status, created_at AS "createdAt", expires_at AS "expiresAt"`; const result = admin ? await db.execute(sql`SELECT ${columns} FROM licenses ORDER BY created_at DESC LIMIT 200`) : await db.execute(sql`SELECT ${columns} FROM licenses WHERE discord_id = ${user.id} ORDER BY created_at DESC`); res.setHeader("Cache-Control", "private, no-store"); return res.json({ licenses: result.rows, isAdmin: admin }); } catch (error) { console.error("[Licenses] read failed", error); return res.status(503).json({ error: "licenses_unavailable" }); }
  });
