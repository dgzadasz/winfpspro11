...
export async function getDb() {
  ...
  if (!_db && process.env.DATABASE_URL) {
    ...
    try {
      ...
      await _db.execute(sql`CREATE TABLE IF NOT EXISTS store_reviews (order_id varchar(32) PRIMARY KEY REFERENCES discord_orders(id), rating integer NOT NULL CHECK (rating BETWEEN 1 AND 5), comment text NOT NULL, sent boolean NOT NULL DEFAULT false, created_at timestamp NOT NULL DEFAULT now())`);
      await _db.execute(sql`CREATE TABLE IF NOT EXISTS support_tickets (id varchar(36) PRIMARY KEY, discord_id varchar(64) NOT NULL, order_id varchar(32) REFERENCES discord_orders(id), subject varchar(120) NOT NULL, message text NOT NULL, status text NOT NULL DEFAULT 'open' CHECK (status IN ('open', 'answered')), reply text, replied_by varchar(64), created_at timestamp NOT NULL DEFAULT now(), updated_at timestamp NOT NULL DEFAULT now())`);
      await _db.execute(sql`CREATE INDEX IF NOT EXISTS support_tickets_owner_idx ON support_tickets (discord_id, created_at DESC)`);
      await _db.execute(sql`CREATE TABLE IF NOT EXISTS licenses (id varchar(40) PRIMARY KEY, order_id varchar(32) NOT NULL UNIQUE REFERENCES discord_orders(id), discord_id varchar(64) NOT NULL, plan varchar(32) NOT NULL, status varchar(16) NOT NULL DEFAULT 'ACTIVE', created_at timestamp NOT NULL DEFAULT now(), expires_at timestamp)`);
      await _db.execute(sql`CREATE INDEX IF NOT EXISTS licenses_discord_id_idx ON licenses (discord_id)`);
      await _db.execute(sql`CREATE TABLE IF NOT EXISTS analytics_events (id bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY, event varchar(40) NOT NULL, product varchar(120), created_at timestamp NOT NULL DEFAULT now())`);
      await _db.execute(sql`CREATE INDEX IF NOT EXISTS analytics_events_created_idx ON analytics_events (created_at DESC)`);
